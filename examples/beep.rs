...
use fundsp::hacker::*;
use fundsp::target_width::TargetF;
    ...
    T: SizedSample + FromSample<TargetF>,
{
    ...
    //let f = 110.0;
    //let m = 5.0;
    //let c = oversample(sine_hz(f as TargetF) * f * m + f >> sine());
    ...
    //let c = lfo(|t: TargetF| xerp11(20.0, 2000.0, sin_hz(0.1, t))) >> dsf_square_r(0.99) >> lowpole_hz(1000.0);
    ...
    // Test ease_noise.
    //let c = lfo(|t: TargetF| xerp11(50.0, 5000.0, ease_noise(smooth9, 0, t))) >> triangle();

    // Bandpass filtering.
    //let c = c >> (pass() | envelope(|t: TargetF| xerp11(500.0, 5000.0, sin_hz(0.05, t)))) >> bandpass_q(5.0);
    //let c = c >> (pass() | envelope(|t: TargetF| (xerp11(500.0, 5000.0, sin_hz(0.05, t)), 0.9))) >> !bandrez() >> bandrez();
    ...
    // Apply Moog filter.
    //let c = (c | lfo(|t: TargetF| (xerp11(110.0, 11000.0, sin_hz(0.1, t)), 0.6))) >> moog();
    ...
    // Add flanger.
    //let c = c
    //    >> (flanger(0.6, 0.005, 0.01, |t: TargetF| lerp11(0.005, 0.01, sin_hz(0.1, t)))
    //        | flanger(0.6, 0.005, 0.01, |t: TargetF| lerp11(0.005, 0.01, cos_hz(0.1, t))));

    // Add phaser.
    //let c = c
    //    >> (phaser(0.5, |t: TargetF| sin_hz(0.1, t) * 0.5 + 0.5)
    //        | phaser(0.5, |t: TargetF| cos_hz(0.1, t) * 0.5 + 0.5));
